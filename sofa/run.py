"""SOFA al ingreso en UCI — pipeline completo.
...
Salidas en `sofa/output/`:
  * `sofa_cohort_<periodo>.csv`   — 1 fila por estancia con valores y SOFA
  * `sofa_summary_<periodo>.csv`  — resumen por unidad/año
  * `sofa_summary_<periodo>.html` — informe estilo demographics
"""
...
from sofa._config import ICU_UNITS, WINDOW_HOURS
from sofa._metrics import compute_sofa, summarize_by_unit_year
from sofa._report import write_cohort_csv, write_html_report, write_summary_csv
from sofa._sql import render_sql

OUTPUT_DIR = _REPO_ROOT / "sofa" / "output"
SNAPSHOT_DIR = _REPO_ROOT / "sofa" / "snapshots"
...
def load_snapshot_or_query(years_str: str, units: list[str],
    ...
    Convención del nombre: `sofa_snapshot_<periodo>.csv` en
    `sofa/snapshots/`. Da igual qué unidades contenga: filtramos en
    Python por las pedidas y por el rango de año.
    """
    snapshot = SNAPSHOT_DIR / f"sofa_snapshot_{years_str}.csv"
    if snapshot.exists():
...
def main():
    ...
    print(f"\nConsultando SOFA {years_str} en {units}...")
    ...
    if len(df) >= 2000 and not (SNAPSHOT_DIR / f"sofa_snapshot_{years_str}.csv").exists():
        ...
        print(f"      el CSV completo en {SNAPSHOT_DIR}/sofa_snapshot_{years_str}.csv")
    ...
    cohort_path = OUTPUT_DIR / f"sofa_cohort_{years_str}.csv"
    summary_path = OUTPUT_DIR / f"sofa_summary_{years_str}.csv"
    html_path = OUTPUT_DIR / f"sofa_summary_{years_str}.html"
    ...
    write_html_report(
        df, summary,
        f"SOFA al ingreso en UCI ({years_str})",
        html_path,
    )
