...
WITH raw_moves AS (
    SELECT 
        ...
        end_date
    FROM g_movements
    WHERE ou_loc_ref IN ({units_formatted})  -- <--- Dynamic Units
      AND start_date BETWEEN '{year}-01-01' AND '{year}-12-31 23:59:59' -- <--- Dynamic Year
),
flagged_starts AS (
    SELECT 
        *,
...
SELECT 
    ...
    CASE WHEN e.exitus_date BETWEEN c.true_start_date AND c.true_end_date THEN 1 ELSE 0 END AS exitus_in_icu,
    CASE 
...
def generate_table(csv_path, year, unit_list):
    ...
    if 'ou_loc_ref' not in df.columns:
        print("ERROR: CSV missing 'ou_loc_ref'. Please check your SQL export.")
        return
    ...
    binary_vars = ['has_cirrhosis', 'exitus_in_icu', 'readmission_at_24h', 'readmission_at_72h']
    ...
    # Replace anything not in top 6 with 'Other'
    df['nationality'] = df['nationality'].where(df['nationality'].isin(top_6_nat), 'Other')
    
    # Sort for the table (Desc frequency)
    df['nationality'] = pd.Categorical(df['nationality'], categories=df['nationality'].value_counts().index, ordered=True)

    # Variables
    columns = ['age_at_start', 'sex', 'nationality', 'los_days', 'has_cirrhosis', 'exitus_in_icu', 'readmission_at_24h', 'readmission_at_72h']
    categorical = ['sex', 'nationality', 'has_cirrhosis', 'exitus_in_icu', 'readmission_at_24h', 'readmission_at_72h']
    nonnormal = ['los_days']
    labels = {
        'age_at_start': 'Age (years)', 'sex': 'Sex', 'nationality': 'Nationality',
        'los_days': 'Length of Stay (days)', 'has_cirrhosis': 'History of Cirrhosis',
        'exitus_in_icu': 'Mortality in ICU', 'readmission_at_24h': 'Readmission (24h)',
        'readmission_at_72h': 'Readmission (72h)'
    }

    # Generate Table
    table_two = TableOne(df, columns=columns, categorical=categorical, groupby='unit', nonnormal=nonnormal, rename=labels, pval=False, missing=False)
    ...
    full_html = f"""<html><head><style>body{{font-family:Arial;margin:40px;}}h2{{color:#2c3e50;}}table{{border-collapse:collapse;width:100%;}}th,td{{border:1px solid #ddd;padding:12px;}}th{{background-color:#f8f9fa;}}</style></head><body><h2>{title_text}</h2>{table_two.to_html()}</body></html>"""
...
def main():
    ...
    # Format for SQL: 'E073', 'I073'
    sql_units_formatted = ", ".join([f"'{u}'" for u in unit_list])
    
    # 2. Modify and Save SQL Query
    query_content = SQL_TEMPLATE.format(year=year_input, units_formatted=sql_units_formatted)
    ...
    while True:
        confirm = input(f"\nHave you saved '{csv_filename}'? (yes/no): ").lower()
        if confirm in ['y', 'yes']:
            if os.path.exists(csv_filename):
                break
            else:
                print(f"ERROR: Could not find file '{csv_filename}'. Please check the name and location.")
        elif confirm in ['n', 'no']:
            print("Exiting. Please run the script again when you have the data.")
            sys.exit()
