...
rass_eligible AS (
    ...
    WHERE r.rc_sap_ref = 'SEDACION_RASS'
      AND (
          ...
          (regexp_like(r.result_txt, '^-?[0-9]+$') AND cast(r.result_txt as bigint) BETWEEN -3 AND 4)
      )
),
...
ORDER BY e.ou_loc_ref, yr,
    CASE e.shift WHEN 'M' THEN 1 WHEN 'A' THEN 2 WHEN 'N' THEN 3 END;
