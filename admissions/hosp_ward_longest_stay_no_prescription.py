"""
HOSPITALISATION WARD STAYS - PREDOMINANT UNIT ASSIGNMENT (NO PRESCRIPTION FILTER)

Admission criteria (both must be met):
...
def get_units_from_user() -> list:
    """Prompt user for units to analyse (accepts single or multiple units)."""
    while True:
        ...
        if len(units) < 1:
            print("❌ Error: You must specify at least 1 unit.")
            print("   Example: E073 or E073,I073\n")
            continue
...
def main():
    print("\n" + "="*70)
    print("  HOSPITALISATION WARD STAYS - PREDOMINANT UNIT ASSIGNMENT")
    print("="*70)
    print("  Each stay assigned to unit with most hours (prevents duplicates)")
    print("  No prescription filter applied")
    print("="*70)
    ...
    if len(units) > 1:
        ...
        print(f"✓ Output for unit {units[0]}\n")
