...
VERSIÓN VAL_ - Usa tablas val_* con nhc y episode_sap
...
FROM grouped_stays
GROUP BY nhc, episode_sap, stay_id;
"""
...
with_prescriptions AS (
    SELECT DISTINCT c.nhc, c.episode_sap, c.stay_id
    FROM cohort c
    INNER JOIN val_prescriptions p 
        ON c.nhc = p.nhc 
        AND c.episode_sap = p.episode_sap
...
LEFT JOIN with_prescriptions wp 
    ON c.nhc = wp.nhc 
    AND c.episode_sap = wp.episode_sap 
    AND c.stay_id = wp.stay_id
WHERE wp.nhc IS NULL
...
LEFT JOIN val_exitus e ON wp.nhc = e.nhc
...
def calculate_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stats = df.groupby('ou_loc_ref').agg(
        total_admissions=('nhc', 'count'),
        unique_patients=('nhc', 'nunique'),
...
def compare_strategies(year: int):
    ...
    print(f"  [VERSIÓN VAL_ - nhc/episode_sap]")
    print(f"{'='*80}")
...
def main():
    ...
    print("║                   [VERSIÓN VAL_ - nhc/episode_sap]            ║")
    print("╚════════════════════════════════════════════════════════════════╝")
    ...
    if option == "1":
        ...
        output_path = output_dir / "summary_strategies_val.csv"
        ...
        if df_no_rx.empty:
            ...
            output_dir = Path(__file__).parent / "output"
            output_dir.mkdir(exist_ok=True)
            df_no_rx_safe = df_no_rx.drop(columns=['nhc', 'episode_sap'], errors='ignore')
            output_path = output_dir / f"no_prescriptions_val_{year}.csv"
        ...
        if df_short.empty:
            ...
            output_dir = Path(__file__).parent / "output"
            output_dir.mkdir(exist_ok=True)
            df_short_safe = df_short.drop(columns=['rango_horas', 'nhc', 'episode_sap'], errors='ignore')
            output_path = output_dir / f"short_stays_val_{max_hours}h_{year}.csv"
        ...
        if df_overlap.empty:
            ...
            if not df_solapamiento.empty:
                ...
                output_dir = Path(__file__).parent / "output"
                output_dir.mkdir(exist_ok=True)
                df_solapamiento_safe = df_solapamiento.drop(columns=['nhc', 'episode_sap'], errors='ignore')
                output_path = output_dir / f"year_overlap_val_{year}.csv"
